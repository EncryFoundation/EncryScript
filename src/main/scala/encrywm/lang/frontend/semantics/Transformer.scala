...
object Transformer {
  ...
  def transform(node: Ast.AST_NODE): Ast.AST_NODE = rewrite(manytd(strategy[Ast.AST_NODE]({
    ...
    case EXPR.Call(EXPR.Attribute(value, attr, _), args, _, _)
      if value.esType.isCollection &&
        attr.name == "exists" &&
        args.size == 1 &&
        args.head.esType.isFunc =>
    ...
    case EXPR.Attribute(value, attr, _)
      if value.esType.isCollection && attr.name == "size" =>
        Some(EXPR.SizeOf(value))

    // Rule: Attribute(coll, "sum") -> SizeOf(coll)
    case EXPR.Attribute(value, attr, _)
      if value.esType.isCollection && attr.name == "sum" =>
      value.esType match {
        case ESList(valT) =>
    ...
    case EXPR.Attribute(value, attr, _)
      if value.esType.isOption && attr.name == "isDefined" =>
